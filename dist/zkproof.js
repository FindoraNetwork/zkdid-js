...
const generateZKProof = async (zkCred, code) => {
    ...
    await tool_1.callApi();
    // The circuit MUST exist and match the `code` provided
    const circuit = circuit_1.getCircuit(zkCred.purpose, code);
...
const _generateSignedZKProof = async (zkCred, code) => {
    ...
    const body = await exports.generateZKProof(zkCred, code);
...
const verifyZKProof = (zkProof, prover, purpose) => {
    ...
    const did = did_1.getDID(prover);
    const zkCred = credential_1.getZKCredential(did, purpose);
    ...
    if (!did_1.didEqual(zkCred.did, did))
    ...
    const circuit = circuit_1.getCircuit(purpose, zkProof.code);
...
const _verifySignedZKProof = (signed, purpose) => {
    ...
    return exports.verifyZKProof(signed.body, address, purpose);
};
