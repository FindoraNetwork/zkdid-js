...
export interface ZKProof {
  code: string; // The circuit code
  proof: string;
  purpose: string;
  commitment: string;
}

// A signed (e.g., by Metamask) ZKProof (useful when DApp needs to verify the ownership of the ZKProof)
 ...
 */
export const verifyZKProof = (prover: string, zkProof: ZKProof): boolean => {
  // Implementation
  //======================basic check
  //
  // 1> Fetch DID by `address` and fetch ZKCredential by `did` and `zkProof.purpose`
  const did = getDID(prover);
  const zkCred = getZKCredential(did, zkProof.purpose);
  //
